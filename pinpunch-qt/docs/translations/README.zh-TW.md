...
本分支在 `pinpunch-qt/src/python/` 目錄下新增了一個 Python 檔案：

```
pinpunch-qt/src/python/taiwan_market_connector.py
```
...
cp taiwan_market_connector.py 路徑/到/pinpunch-qt/src/python/
```
...
找到 `pinpunch-qt/src/python/` 目錄中的連接器登錄檔案（通常是 `data_sources.py` 或 `connectors/__init__.py`），加入以下程式碼：
...
```bash
# 在專案根目錄執行
python pinpunch-qt/src/python/taiwan_market_connector.py
```
